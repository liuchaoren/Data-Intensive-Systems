...
#master = "local[4]"
master = "spark://ec2-54-167-24-226.compute-1.amazonaws.com:7077"
...
#    return (key + ":" + str(start) + ":" + str(end), (agree, disagree))
...
#    results = votes_intervals.reduceByKey(reduce_count).cartesian(big_ints_rdd).filter(filter_by_int_big).map(key_by_interval_and_person_big).reduceByKey(reduce_count)
...
#    results = run(context)
    ...
    output.write("Persons (timeStart timeEnd) agree# disagree#\n")
    for result in results:
        (persons, (start, end, agree, disagree)) = result
        output.write("%s\t(%s\t%s)\t%d\t%d\n" % (persons, str(start), str(end), agree, disagree))
    output.close()
